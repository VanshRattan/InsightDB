...
def create_agent_graph(pool) -> Any:
    ...
    if pool is not None:
        try:
            ...
            logger.warning(f"Postgres checkpointer unavailable, using in-memory checkpoints: {e}")
            checkpointer = None

    if checkpointer is None:
        from langgraph.checkpoint.memory import InMemorySaver
        logger.warning("Using in-memory LangGraph checkpointer (conversation state is not persisted across restarts).")
