...
async def lifespan(app: FastAPI):
    ...
    if POSTGRES_ENABLED:
        ...
        try:
            ...
            logger.warning(f"PostgreSQL is unavailable; continuing with in-memory state: {e}")
            db_pool = None
    else:
        logger.info("PostgreSQL is disabled; using in-memory state.")
        db_pool = None
...
@app.get("/")
async def root():
    return {"status": "ok", "service": "DataAgent Pro", "docs": "/docs"}
